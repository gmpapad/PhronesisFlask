...
PERSPECTIVES_DIR = Path(__file__).parent / "content" / "perspectives"

def load_all_perspectives():
    """Read every *.json in content/perspectives and return a sorted list."""
    items = []
    ...
    for f in PERSPECTIVES_DIR.glob("*.json"):
        try:
            ...
            # Optional: log and skip malformed files
            app.logger.warning(f"Skipping {f.name}: {e}")
            continue
    # sort by order then title
    items.sort(key=lambda x: (x["order"], x["title"]))
    return items

def get_perspective_progress(user_id: int, perspective_slug: str, total_lessons: int) -> int:
    """Compute % completed based on Progress rows for this perspective."""
    if total_lessons <= 0:
        return 0
    completed = (
    ...
    pct = round(100 * min(completed, total_lessons) / total_lessons)
    return pct
